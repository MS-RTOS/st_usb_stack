...
USBH_StatusTypeDef  USBH_Init(USBH_HandleTypeDef *phost, void (*pUsrFunc)(USBH_HandleTypeDef *phost, uint8_t id), uint8_t id)
{
  ...
  void *msg_buf = ms_kmalloc(MSGQUEUE_OBJECTS * sizeof(ms_uint32_t));

  if (msg_buf != MS_NULL) {
      ms_mqueue_create("usbh_mq", msg_buf, MSGQUEUE_OBJECTS, sizeof(ms_uint32_t),
                       MS_WAIT_TYPE_PRIO, &phost->os_event);

      ms_thread_create("t_usbh", USBH_Process_OS, phost,
                       USBH_PROCESS_STACK_SIZE, USBH_PROCESS_PRIO, USBH_PROCESS_TIME_SLICE,
                       MS_THREAD_OPT_SUPER, &phost->thread);
  }
